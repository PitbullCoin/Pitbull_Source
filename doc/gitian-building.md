...
*Setup instructions for a Gitian build of Pitbullcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Pitbullcoin
...
- [Building Pitbullcoin](#building-pitbullcoin)
...
Clone the git repositories for pitbullcoin and Gitian and then checkout the pitbullcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/pitbullcoin-project/pitbullcoin.git
cd pitbullcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the pitbullcoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for Gitian to work offline.

Building Pitbullcoin
----------------

To build Pitbullcoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the pitbullcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pitbullcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/pitbullcoin-project/pitbullcoin
...
For example:
```bash
URL=https://github.com/user/pitbullcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit pitbullcoin=${COMMIT} --url pitbullcoin=${URL} ../pitbullcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pitbullcoin=${COMMIT} --url pitbullcoin=${URL} ../pitbullcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pitbullcoin=${COMMIT} --url pitbullcoin=${URL} ../pitbullcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pitbullcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pitbullcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/pitbullcoin-build.assert
```
...
[pitbullcoin/gitian.sigs](https://github.com/pitbullcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
